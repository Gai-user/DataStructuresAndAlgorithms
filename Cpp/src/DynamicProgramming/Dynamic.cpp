 ...
 * @return 公共子串长度 
 */
int Dynamic::LongestCommonSubStringLength(const std::string& word1, const std::string& word2)
{
    ...
    std::vector<std::vector<int>> maxLen(word1Len + 1, std::vector<int>(word2Len + 1, 0));

    // 两个空字符串的公共子串长度为0
    for (int i = 1; i <= word1Len; i++) {
        for (int j = 1; j <= word2Len; j++) {
            if (word1[i - 1] == word2[j - 1]) {
                maxLen[i][j] = std::max({maxLen[i-1][j-1], maxLen[i][j-1], maxLen[i-1][j]}) + 1;
            } else {
                maxLen[i][j] = std::max({maxLen[i-1][j-1], maxLen[i][j-1], maxLen[i-1][j]});
            }
        }
    }
    return maxLen[word1Len][word2Len];
}